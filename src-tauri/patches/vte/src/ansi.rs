...
pub trait Handler {
    ...
    ///
    /// `command` is the marker character: 'A' (prompt start), 'B' (command start),
    /// 'C' (command executed / output start), 'D' (command finished).
    /// `params` carries any trailing parameters (e.g. exit code after 'D').
    fn osc133(&mut self, _command: char, _params: &str) {}
    ...
    T: Timeout,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // Shell integration / semantic prompt (OSC 133).
            b"133" => {
                if params.len() >= 2 && !params[1].is_empty() {
                    let command = params[1][0] as char;
                    let rest = if params[1].len() > 1 && params[1][1] == b';' {
                        str::from_utf8(&params[1][2..]).unwrap_or("")
                    } else if params.len() > 2 {
                        str::from_utf8(params[2]).unwrap_or("")
                    } else {
                        ""
                    };
