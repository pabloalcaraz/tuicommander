...
import { isMacOS, isQuickSwitcherActive } from "../platform";
...
function dispatchAction(action: ActionName, handlers: ShortcutHandlers): boolean {
	switch (action) {
		...
		// Split panes
		case "split-vertical":
			handlers.handleSplit("vertical");
			return true;
		case "split-horizontal":
			handlers.handleSplit("horizontal");
			return true;
		...
		case "quick-branch-switch":
			...
			return true;
		case "toggle-error-log":
			handlers.toggleErrorLog();
			return true;
		case "toggle-branches-tab":
			handlers.toggleBranchesTab();
			return true;
		case "toggle-ai-chat":
			if (settingsStore.isAiChatEnabled()) handlers.toggleAiChatPanel();
			return true;
		case "toggle-mcp-popup":
			handlers.toggleMcpPopup();
			return true;
		case "clear-scrollback":
			handlers.clearScrollback();
			return true;
		case "scroll-to-top":
		...
		default: {
			...
			if (branchMatch) {
				handlers.switchToBranchByIndex(parseInt(branchMatch[1], 10));
				return true;
			}
...
export function useKeyboardShortcuts(handlers: ShortcutHandlers): () => void {
	const handleKeydown = (e: KeyboardEvent) => {
		...
		if (isQuickSwitcherActive(e) && e.key >= "1" && e.key <= "9") {
			e.preventDefault();
			handlers.switchToBranchByIndex(parseInt(e.key, 10));
			return;
		}
