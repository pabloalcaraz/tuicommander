...
export function useTerminalLifecycle(deps: TerminalLifecycleDeps) {
	...
	const selectAfterNonTerminalClose = (
		store: {
			getIds: () => string[];
			getVisibleIds: (key: string | null) => string[];
	...
	const closeTerminal = async (id: string, skipConfirm = false) => {
		...
		setClosedTabs((prev) => [
			...prev.slice(-9),
			{ name: terminal.name, fontSize: terminal.fontSize, cwd: terminal.cwd },
		]);
		...
		if (wasActive) {
			...
			if (nextId) {
				...
				requestAnimationFrame(() => {
					if (
						terminalsStore.get(nextId) ||
						nextId.startsWith("diff-") ||
						nextId.startsWith("md-") ||
						nextId.startsWith("edit-")
					) {
