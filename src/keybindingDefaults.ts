...
export const DEFAULT_BINDINGS: Record<ActionName, string> = {
	...
	"prev-tab": "",
	"next-tab": "",
	"focus-last-terminal": "Cmd+Ctrl+Backspace",
	"jump-waiting-terminal": "Cmd+U",
	"clear-terminal": "Cmd+L",
	"refresh-terminal": "Cmd+Shift+L",
	"toggle-git-ops": "Cmd+Shift+D",
	// "Cmd+?" never matched: pressing ? holds Shift, so the event normalizes to
	// "cmd+shift+/" (eventToCombo un-shifts ?→/). Express it that way so it fires.
	"toggle-help": "Cmd+Shift+/",
	...
	...Object.fromEntries(Array.from({ length: 9 }, (_, i) => [`switch-branch-${i + 1}`, `Cmd+Ctrl+${i + 1}`])),
