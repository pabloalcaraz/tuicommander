...
function mockPr(
	overrides: Partial<BranchPrStatus> & Pick<BranchPrStatus, "number" | "branch" | "state">,
): BranchPrStatus {
	return {
		title: "",
		url: "",
...
describe("WorktreeManager", () => {
	...
	it("lists worktrees from repositories store", () => {
		...
		const branchTexts = Array.from(rows).map((r) => r.querySelector("span[class*='branch']")?.textContent);
