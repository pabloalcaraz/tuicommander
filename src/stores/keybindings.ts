...
export function createKeybindingsStore() {
	...
	function rebuildMaps(bindings: Record<string, string | undefined>) {
		...
		for (const [action, key] of Object.entries(bindings)) {
			...
			if (!comboToAction.has(normalized)) {
				...
				appLogger.warn("config", `Keybinding conflict on "${key}": "${action}" skipped (already bound to "${winner}")`);
			}
		}
	}
	...
	function isKnownAction(action: string): boolean {
		return (ACTION_NAMES as readonly string[]).includes(action) || dynamicActions.has(action);
	}
		...
		async hydrate(): Promise<void> {
			try {
				...
				for (const entry of overrides) {
					...
					if (!(ACTION_NAMES as readonly string[]).includes(entry.action) && !entry.action.startsWith("plugin:")) {
						continue;
					}
