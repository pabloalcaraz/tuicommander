...
function createTerminalsStore() {
	...
	function handleShellStateChange(id: string, prev: ShellState, next: ShellState): void {
		if (next === "busy" && prev !== "busy") {
			...
			// instead clear when the backend's authoritative "awaiting" event flips false
			// (user-input / status-line reconciliation, 060) or on process exit (below).
	...
	const actions = {
		 ...
		 *
		 * Story 060: the backend accumulator is the source of truth for the
		 * question badge and pushes an authoritative "awaiting" event on every
		 * change (sticky while the prompt is up, cleared on user-input). The
		 * frontend no longer runs a sustained-busy timer to second-guess it. */
