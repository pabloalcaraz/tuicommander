								...
								fallback={
									<div class={s.empty}>
										...
										<button onClick={createNewPrompt}>
											{t("promptDrawer.createFirst", "Create your first prompt")}
										</button>
									</div>
								}
							>
												...
												<div class={s.itemContent}>
													<div class={s.itemName}>
														...
														<span class={s.itemBadges}>
															<span class={s.tagBadge} data-type={prompt.executionMode ?? "inject"}>
																{prompt.executionMode ?? "inject"}
															</span>
															<Show when={prompt.builtIn}>
																<span class={s.tagBadge} data-type="builtin">
																	built-in
																</span>
															</Show>
															<For each={prompt.placement ?? []}>
																{(p) => (
																	<span class={s.tagBadge} data-type="placement">
																		{p}
																	</span>
																)}
															</For>
					...
					<div class={s.variableOverlay} onClick={() => setShowVariableDialog(false)}>
						<div class={s.variableDialog} onClick={(e) => e.stopPropagation()}>
								...
								{(varName) => (
									<div class={s.variableInput}>
										...
										<input
											...
											onInput={(e) => setVariableValues((v) => ({ ...v, [varName]: e.currentTarget.value }))}
							...
							<div class={s.variableActions}>
								...
								<button onClick={() => handleVariableSubmit(true)}>
									{t("promptDrawer.insertAndRun", "Insert & Run")}
								</button>
...
const VariableDropdown: Component<{
	...
	return (
		<div class={s.varDropdownWrap}>
			<button class={s.varDropdownBtn} onClick={() => setOpen(!open())} type="button">
				Insert variable...
				<span class={s.varChevron} classList={{ [s.varChevronOpen]: open() }}>
					&#9660;
				</span>
			</button>
			<Show when={open()}>
		...
		<div class={s.editorOverlay} onClick={props.onCancel}>
			<div class={s.editor} onClick={(e) => e.stopPropagation()}>
				<h4>
					...
					<Show when={isBuiltIn()}>
						<span class={s.badge} style={{ "margin-left": "8px" }}>
							built-in
						</span>
					</Show>
				</h4>
					...
					<Show when={executionMode() === "inject"}>
						<div class={s.editorField}>
							...
							<label class={s.toggleRow}>
								<input type="checkbox" checked={autoExecute()} onChange={() => setAutoExecute(!autoExecute())} />
				...
				<div class={s.editorField}>
					...
					<KeyComboCapture value={shortcut()} onChange={setShortcut} placeholder="Click to set shortcut" />
				</div>

				{/* Actions */}
				<div class={s.editorActions}>
					<Show when={isBuiltIn() && isOverridden()}>
						<button class={s.resetBtn} onClick={handleReset}>
							Reset to Default
						</button>
					</Show>
					<div style={{ flex: "1" }} />
