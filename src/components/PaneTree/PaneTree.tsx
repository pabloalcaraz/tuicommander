	...
	const handleMouseDown = (handleIndex: number, startEvent: MouseEvent) => {
		...
		const onMouseMove = (e: MouseEvent) => {
			...
			requestAnimationFrame(() => {
				rafPending = false;
				const fraction = vertical ? (e.clientX - rect.left) / rect.width : (e.clientY - rect.top) / rect.height;
		...
		>
           ...
           imperative state (xterm/WebGL) that must persist across tab switches. */}
			<div class="pane-content">
				<Show
					when={aliveTabs().length > 0}
					fallback={<PanePlaceholder onNewTerminal={() => props.onNewTerminal?.(props.groupId)} />}
				>
					<For each={aliveTabs()}>
						{(tab) => (
