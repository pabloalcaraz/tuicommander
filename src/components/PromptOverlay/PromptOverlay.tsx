...
export const PromptOverlay: Component<PromptOverlayProps> = (props) => {
	...
	createEffect(() => {
		...
		const handleKeydown = (e: KeyboardEvent) => {
			...
			switch (e.key) {
				...
				case "ArrowDown":
					e.preventDefault();
					setSelectedIndex((i) => Math.min(currentPrompt.options.length - 1, i + 1));
					break;
				case "Enter":
					e.preventDefault();
					confirm();
	...
	return (
		<div class={cx(s.overlay, !isVisible() && s.hidden)}>
			<Show when={prompt()}>
				<div class={s.dialog}>
					<div class={s.question}>{prompt()?.question || t("promptOverlay.defaultQuestion", "Select an option:")}</div>
					<div class={s.options}>
					...
					<div class={s.hint}>
						{t("promptOverlay.hint", "Press 1-{count} or Enter to select, Escape to cancel", {
							count: String(prompt()?.options.length ?? 0),
						})}
					</div>
				</div>
			</Show>
