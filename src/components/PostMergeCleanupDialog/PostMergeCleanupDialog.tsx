...
export const PostMergeCleanupDialog: Component<PostMergeCleanupDialogProps> = (props) => {
	...
	const toggleStep = (id: StepId) => {
		...
		setSteps((prev) =>
			prev.map((step) => (step.id === id && !step.disabled ? { ...step, checked: !step.checked } : step)),
		);
	};
	...
	return (
		<div class={d.overlay} onClick={props.onSkip}>
			<div class={d.popover} onClick={(e) => e.stopPropagation()}>
									...
									<li>
										<div class={s.step}>
											<label class={step.disabled ? s.disabled : undefined}>
												...
												/>
												<Show
													when={step.id === "worktree" && props.onWorktreeActionChange}
											...
											<Show when={status() !== "pending"}>
												<span class={`${s.stepStatus} ${s[status()]}`} data-testid={`step-status-${step.id}`}>
													{STATUS_ICONS[status()]}
												</span>
											</Show>
										</div>
				...
				<div class={d.actions}>
					<button class={d.cancelBtn} data-testid="skip-btn" onClick={props.onSkip} disabled={!!props.executing}>
						Skip
					</button>
					<button class={d.primaryBtn} data-testid="execute-btn" onClick={handleExecute} disabled={!!props.executing}>
						Execute
					</button>
				</div>
			</div>
