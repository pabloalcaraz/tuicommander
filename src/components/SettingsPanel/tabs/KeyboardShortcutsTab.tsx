	...
	return [
		{
			...
			shortcuts: [
				{
					action: "new-terminal",
					keys: keyFor("new-terminal"),
					description: t("helpPanel.newTerminalTab", "New terminal tab"),
				},
				{
					action: "close-terminal",
					keys: keyFor("close-terminal"),
					description: t("helpPanel.closeTerminalTab", "Close terminal tab"),
				},
				{
					action: "run-command",
					keys: keyFor("run-command"),
					description: t("helpPanel.runSavedCommand", `Run saved command (${editKey} to edit)`),
				},
				{ action: "edit-command", keys: keyFor("edit-command"), description: "Edit saved command" },
				{
					action: "reopen-closed-tab",
					keys: keyFor("reopen-closed-tab"),
					description: t("helpPanel.reopenClosedTab", "Reopen closed tab"),
				},
				{
					keys: keyFor("switch-tab-1", `${mod}1-9`),
					description: t("helpPanel.switchToTab", "Switch to tab by number"),
				},
				...
				{ keys: isMacOS() ? "⌃Tab" : "Ctrl+Tab", description: t("helpPanel.nextTab", "Next tab") },
				{
					...
					keys: keyFor("clear-terminal"),
					description: t("helpPanel.clearTerminal", "Clear terminal"),
				},
				{
					action: "find-in-terminal",
					keys: keyFor("find-in-terminal"),
					description: t("helpPanel.findInContent", "Find in content"),
				},
		...
		},
		{
			...
			shortcuts: [
				...
				{
					action: "zoom-in-all",
					keys: keyFor("zoom-in-all"),
					description: t("helpPanel.zoomInAll", "Zoom in all terminals"),
				},
				{
					action: "zoom-out-all",
					keys: keyFor("zoom-out-all"),
					description: t("helpPanel.zoomOutAll", "Zoom out all terminals"),
				},
				{
					action: "zoom-reset-all",
					keys: keyFor("zoom-reset-all"),
					description: t("helpPanel.resetZoomAll", "Reset zoom all terminals"),
				},
			],
		},
		{
			...
			shortcuts: [
				{
					...
					keys: keyFor("toggle-markdown"),
					description: t("helpPanel.toggleMarkdownPanel", "Toggle markdown panel"),
				},
				{
				...
				},
				{
					...
					keys: keyFor("toggle-settings"),
					description: t("helpPanel.openSettings", "Open settings"),
				},
				{
					action: "toggle-task-queue",
					keys: keyFor("toggle-task-queue"),
					description: t("helpPanel.toggleTaskQueue", "Toggle task queue"),
				},
				{ action: "toggle-error-log", keys: keyFor("toggle-error-log"), description: "Toggle error log" },
				...(settingsStore.isAiChatEnabled()
					? [{ action: "toggle-ai-chat" as const, keys: keyFor("toggle-ai-chat"), description: "Toggle AI Chat" }]
					: []),
				...
				{
					action: "toggle-notes",
					keys: keyFor("toggle-notes"),
					description: t("helpPanel.toggleIdeasPanel", "Toggle ideas panel"),
				},
				{
					action: "toggle-help",
					keys: keyFor("toggle-help"),
					description: t("helpPanel.toggleHelpPanel", "Toggle help panel"),
				},
				{
				...
				},
				{
					...
					description: t("helpPanel.activityDashboard", "Activity dashboard"),
				},
				{ action: "prompt-library", keys: keyFor("prompt-library"), description: "Prompt library" },
				{
					action: "toggle-global-workspace",
					keys: keyFor("toggle-global-workspace"),
					description: "Toggle global workspace",
				},
				{
					action: "toggle-focus-mode",
					keys: keyFor("toggle-focus-mode"),
			...
			shortcuts: [
				{
					...
					keys: keyFor("split-vertical"),
					description: t("helpPanel.splitVertically", "Split vertically (side by side)"),
				},
				{
					action: "split-horizontal",
					keys: keyFor("split-horizontal"),
					description: t("helpPanel.splitHorizontally", "Split horizontally (stacked)"),
				},
				{
					keys: "\u2325\u2190/\u2192",
					description: t("helpPanel.navigatePanesVertical", "Navigate panes (vertical split)"),
				},
				{
					keys: "\u2325\u2191/\u2193",
					description: t("helpPanel.navigatePanesHorizontal", "Navigate panes (horizontal split)"),
				},
				{
					action: "close-terminal",
					keys: keyFor("close-terminal"),
					description: t("helpPanel.closeActivePane", "Close active pane (or tab if single)"),
				},
			],
		},
		{
			...
			shortcuts: [
				{
					...
					description: t("helpPanel.toggleSidebar", "Toggle sidebar"),
				},
				{
					keys: keyFor("switch-branch-1", `${mod}^1-9`),
					description: t("helpPanel.quickSwitchBranch", "Quick switch to branch"),
				},
				{ keys: `Hold ${mod}^`, description: t("helpPanel.showQuickSwitcher", "Show quick switcher") },
				{
					keys: t("helpPanel.dragRepo", "Drag repo"),
					description: t("helpPanel.reorderRepos", "Reorder repos or move between groups"),
				},
				{ keys: t("helpPanel.dragGroup", "Drag group"), description: t("helpPanel.reorderGroups", "Reorder groups") },
				{
					keys: t("helpPanel.rightClickGroup", "Right-click group"),
					description: t("helpPanel.renameColorDelete", "Rename, change color, delete"),
				},
			],
		},
		{
			...
			shortcuts: [
				...
				},
				{
					...
					keys: keyFor("toggle-file-browser-content-search"),
					description: "Search file contents",
				},
				{ action: "open-file", keys: keyFor("open-file"), description: "Open file…" },
				{ action: "new-file", keys: keyFor("new-file"), description: "New file…" },
				{ action: "open-folder", keys: keyFor("open-folder"), description: "Open folder…" },
				{ action: "open-path", keys: keyFor("open-path"), description: "Open path…" },
		...
		},
		{
			...
			shortcuts: [
				{
					keys: "Hold F5",
					description: t("helpPanel.pushToTalk", "Push-to-talk, works globally (configurable in Settings)"),
				},
				{
					keys: t("helpPanel.holdMicBtn", "Hold Mic btn"),
...
export const KeyboardShortcutsTab: Component = () => {
		...
		return getShortcutSections()
			.map((section) => ({
				...
				shortcuts: section.shortcuts.filter(
					(sc) => sc.keys.toLowerCase().includes(q) || sc.description.toLowerCase().includes(q),
				),
			}))
	...
	function comboToTauri(combo: string): string {
		return combo.replace(/\bCmd\b/g, "Super").replace(/\bCtrl\b/g, "Control");
	}

	/** Convert Tauri format back to display format */
	function tauriToDisplay(combo: string): string {
		return combo.replace(/\bSuper\b/g, "Cmd").replace(/\bControl\b/g, "Ctrl");
	}
								...
								{(shortcut) => (
									<tr style={{ cursor: "default" }}>
										<td style={{ width: "140px", padding: "4px 8px", "vertical-align": "middle" }}>
											<Show
												when={editingAction() === shortcut.action}
												fallback={
													<div style={{ display: "flex", "align-items": "center", gap: "4px" }}>
														<kbd
															style={{
																...
																padding: "2px 8px",
																background:
																	shortcut.action && keybindingsStore.isOverridden(shortcut.action)
										...
										<Show
											...
											fallback={
												<div style={{ display: "flex", "align-items": "center", gap: "4px" }}>
													...
													>
														{cmd.keys || t("helpPanel.unbound", "unbound")}
													</kbd>
													<button
...
const RecordingIndicator: Component<{
	...
	return (
		<div style={{ display: "flex", "flex-direction": "column", gap: "4px" }}>
			...
			>
				{props.conflict ? comboToDisplay(props.conflict.combo) : t("helpPanel.pressKey", "Press a key...")}
			</div>
