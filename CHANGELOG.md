...
### Fixed
- **Background agent commands no longer look complete at a ready prompt** — Session lifecycle now keeps `agent_state=working` and defers parent idle mail while meaningful descendants such as Cargo/test processes remain alive, without conflating that task state with terminal input readiness. Persistent integration helpers do not pin agents working.
