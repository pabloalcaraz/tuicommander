...
- **Codex MCP spawn and submit reliability** — `agent action=spawn` composes structured `model` with explicit `args`, includes Codex's approval-bypass default, and no longer drops the required task when callers pass flags. Interactive Codex/OpenCode sessions defer that task through the existing ready-screen injection path. Combined MCP `session input` text + Enter now uses the required split-write gap for those prefill-only TUIs instead of leaving text unsubmitted in the composer. Claude's established input path and default spawn argv remain unchanged.
